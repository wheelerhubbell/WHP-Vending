...
export class PaymentRail {
  ...
  async readJsonResponse(response, code) {
    if (!response.ok) {
  ...
  async postJson(url, body, code) {
    ...
    return this.readJsonResponse(response, code);
  }

  async supported() {
    let response;
    try {
      response = await this.fetch(`${this.facilitatorUrl}/supported`, {
        method: "GET",
      ...
      throw new Fault("PAYMENT_PROVIDER_UNAVAILABLE", 503, { retryable: true });
    }
    const value = await this.readJsonResponse(response, "PAYMENT_PROVIDER_UNAVAILABLE");
    demand(Array.isArray(value?.kinds) && Array.isArray(value?.extensions), "PAYMENT_PROVIDER_CAPABILITIES_INVALID", 503);
    const exactBaseMainnet = value.kinds.some((kind) => kind?.x402Version === 2 && kind?.scheme === "exact" && kind?.network === BASE_NETWORK);
    const bazaar = value.extensions.includes("bazaar");
    demand(exactBaseMainnet, "PAYMENT_PROVIDER_BASE_MAINNET_UNSUPPORTED", 503);
    return { exact_base_mainnet: true, bazaar };
  }
  ...
  async finalizedHead() {
    ...
    demand(head && /^0x[0-9a-fA-F]+$/u.test(head.number ?? "") && HEX32.test(head.hash ?? "")
      && /^0x[0-9a-fA-F]+$/u.test(head.timestamp ?? ""), "RPC_FINALITY_UNSUPPORTED", 503);
    return { number: Number(BigInt(head.number)), hash: head.hash.toLowerCase(), timestamp: Number(BigInt(head.timestamp)) };
  }
  ...
  async reconcile(row, observedAt) {
    ...
    if (from > to) {
      if (BigInt(head.timestamp) > BigInt(authorization.validBefore)) {
        return {
          expired_unsettled: true,
          network: payment.accepted.network,
          payer: authorization.from.toLowerCase(),
          nonce: authorization.nonce.toLowerCase(),
          valid_before: authorization.validBefore,
          finalized_head: { number: head.number, hash: head.hash, timestamp: head.timestamp },
          observed_at: observedAt,
        };
      }
      return null;
    ...
    for (const log of logs) {
      demand(HEX32.test(log?.transactionHash ?? ""), "CHAIN_LOGS_INVALID", 503);
      const transaction = log.transactionHash.toLowerCase();
      const proof = await this.evidence(payment, log.transactionHash, observedAt);
      if (proof) return proof;
      demand(candidateTransaction === null || candidateTransaction === transaction, "CHAIN_LOGS_AMBIGUOUS", 503);
      candidateTransaction = transaction;
    }
    if (candidateTransaction) return { candidate_transaction: candidateTransaction };
    if (to === finalizedNumber && BigInt(head.timestamp) > BigInt(authorization.validBefore)) {
      return {
        expired_unsettled: true,
        network: payment.accepted.network,
        payer: authorization.from.toLowerCase(),
